# monnaie-works.github.io

monnaie works のGitHub Pages用サイト一式です。

## 公開URL

https://monnaie-works.github.io/

## 受付方針

現段階では、新規の依頼・見積もり・契約・決済はランサーズ経由に集約します。

- ランサーズプロフィール: https://www.lancers.jp/profile/monnaie_works
- noteマガジン: https://note.com/philo_monnaie/m/m2eb912a5b10f
- Protocol Order: https://protocolorder.github.io/protocolorder-teaser/

## 今回の主な更新内容

- 2020年から金融記事を中心に執筆していることを明記
- 暗号資産関連の商業書籍の執筆実績を追加
- 契約上、具体的な掲載先名・担当範囲を公開できない実績がある旨を追加
- FP2級対策講座、日商簿記2級対策講座の講師経験を明記
- TOEIC865、英語一次情報リサーチ、フランス語・イタリア語資格を追加
- 暗号資産、外貨、国内外株式・ETF、投資信託、収益不動産などの資産運用経験を追加
- ブロックチェーンを信用・所有・制度・プロトコルの観点から整理できる訴求に修正
- Protocol Orderを関連公開プロジェクトとして追加
- 金融記事に関する免責・対応範囲を追加
- 依頼前に共有してほしい情報、納品形式・対応ツールを追加

## ファイル構成

```txt
.
├── index.html
├── profile.html
├── services.html
├── portfolio.html
├── rates.html
├── policy.html
├── contact.html
├── samples/
│   ├── crypto-beginner.html
│   ├── exchange-choice.html
│   └── structure-sample.html
├── assets/
│   ├── style.css
│   └── favicon.svg
├── robots.txt
├── sitemap.xml
├── .nojekyll
└── README.md
```

## アップロード手順

1. GitHubの `monnaie-works.github.io` リポジトリを開く
2. `Add file` → `Upload files` を選択
3. このフォルダ内のファイルとフォルダをすべてアップロード
4. Commit message に `Update portfolio site` と入力
5. `Commit changes` を実行

## 注意

直取引フォームやメールアドレスはまだ掲載していません。


## 2026-06-12 update

- contact.html にご依頼フォーマットを追加。
- contact.html / policy.html / services.html に、構成が決まっている場合・構成から依頼する場合の進行方針を明記。
- 面談や複数回の企画相談、複数案の提案・再提案を前提とする形式は原則非対応と明記。
- トンマナ・表記ルールは、指定がない場合はです・ます調の丁寧な解説調で進める方針を追加。
- policy.html の修正規定を「当初の制作条件から大きく変更となる修正」に整理。
